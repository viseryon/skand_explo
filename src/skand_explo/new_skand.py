...
class Analyser:
    ...
    def _export_table(
        ...
        if world_data:
            nr_formats = dict.fromkeys(table.keys(), "{:,.4%}")
        else:
            nr_formats = {x: "{:,.1f}" if int(x) > 1_000 else "{:.1%}" for x in table.keys()}
        ...
        subset = pd.IndexSlice[[indx for indx in table.index if indx != "WORLD"], :]
